...
import { mockFolders, getMockData } from "@/data/mocks";
...
const Uploads = () => {
    ...
    try {
      for (const file of pendingFiles) {
        ...
        if (destinationType === 'new_client') {
          ...
          // Aqui criaria a nova pasta de cliente
        } else if (destinationType === 'new_subfolder') {
          await new Promise(resolve => setTimeout(resolve, 500));
          // Aqui criaria a nova subpasta
        }
  ...
  const allFolders = folders.filter(f => f.kind !== 'client'); // subpastas e casos
